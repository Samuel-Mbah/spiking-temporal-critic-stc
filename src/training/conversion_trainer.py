...
def run_conversion(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ann_agent = make_agent(
        ...
        in_dim=config.get("model", {}).get("in_features", 4),
        ...
        detach_critic_for_actor=bool(config.get("model", {}).get("detach_critic_for_actor", True)),
        normalize_critic_for_actor=bool(config.get("model", {}).get("normalize_critic_for_actor", True)),
        critic_actor_value_clip=config.get("model", {}).get("critic_actor_value_clip", 5.0),
        critic_actor_norm_momentum=float(config.get("model", {}).get("critic_actor_norm_momentum", 0.01)),
    ).to(device)
    ...
    # --- Research Standard State (Phase 1) ---
    ...
    if start_update > total_updates:
        logger_mod.info(
            ...
            "skipping additional ANN training updates and proceeding to conversion."
        )
    ...
    for update in range(start_update, total_updates + 1):
        ...
        train_dynamic_joules = energy_hook.energy.get("dynamic_joules", train_joules)
        train_full_joules = float(train_full_stats.get("total_joules", 0.0))
        train_full_dynamic_joules = float(train_full_stats.get("dynamic_joules", train_full_joules))
        ...
        logger.record("energy/train_rollout_dynamic", train_dynamic_joules)
        ...
        steps_in_rollout = train_cfg.get("rollout_length", 2048) * n_envs
        if steps_in_rollout > 0:
            logger.record("latency/mean_ms", (duration / steps_in_rollout) * 1000.0)
        ...
        if update % eval_interval == 0 or update == total_updates:
            ...
            te, success_rate, avg_len, success_count, n_eval_episodes = ann_trainer.evaluate()
            inf_res = energy_hook.stop_span(eval_meter, eval_t0)

            inf_joules = inf_res.get("total_joules", 0.0)
            inf_dynamic_joules = inf_res.get("dynamic_joules", inf_joules)
            cumulative_energy += inf_joules
            cumulative_dynamic_energy += inf_dynamic_joules
            logger.record("energy/inference", inf_joules)
            logger.record("energy/inference_dynamic", inf_dynamic_joules)
            logger.record("energy/total", cumulative_energy)
            logger.record("energy/total_dynamic", cumulative_dynamic_energy)
            ...
            # --- 2. Log Solution (No Break) ---
            if is_solved and not has_logged_ann_solution:
                logger_mod.info(f"✅ ANN solved environment at update {update} (Avg: {rolling_avg:.2f})! Continuing training for stability...")
    ...
    if hasattr(ann_agent.actor, "backbone"):
        # Keep action head dimension consistent with the trained ANN actor.
        action_dim = int(ann_agent.actor.policy_head.out_features)
        actor_snn = Actor(
            actor_snn_backbone,
            latent_dim=config.get("model", {}).get("hidden_dim", 64),
    ...
    # Critic Verification (Conditional)
    critic_metrics = {}
    if has_snn_critic:
        ...
        # Backward/forward compatibility: some verifier versions do not emit max_error.
    ...
    # 3. VISUALIZATION (Plotting Both)
    try:
        with torch.no_grad():
            # --- A. Actor Data Collection ---
            ...
            if has_snn_critic:
                ...
                # SNN Value (Handle temporal forward)
                if hasattr(agent_snn.critic, "forward_T"):
                    # forward_T typically returns (values, ..., ...)
        ...
        # --- Generate Actor Plot ---
        convert_cfg = config.get("snn_convert", {})
        convert_actor = bool(convert_cfg.get("convert_actor", True))
        convert_critic = bool(convert_cfg.get("convert_critic", False))
        ...
        plot_conversion_validation(
            ann_values=ann_actor_out_np,
            snn_values=snn_actor_out_np,
            save_path=actor_viz_path,
            exp_name=conv_exp_name,
            env_name=conv_env_name,
            component="Actor",
        )
        logger_mod.info(f"Saved Actor conversion plot to: {actor_viz_path}")

        # --- Generate Critic Plot (New) ---
        if not has_snn_critic:
            ...
            # We reuse the same plotting function since it just plots X vs Y
            plot_conversion_validation(
                ann_values=ann_critic_out.cpu().numpy(),
                snn_values=snn_critic_out.cpu().numpy(),
                save_path=critic_viz_path,
                exp_name=conv_exp_name,
                env_name=conv_env_name,
                component="Critic",
            )
    ...
    sticky_cfg = config.get("sticky_action", {})
    n_eval_episodes = int(ppo_cfg.get("eval_episodes", 5))
    logger_mod.info(f"Zero-shot eval over {n_eval_episodes} episodes...")
    ...
    zs_dynamic_joules = float(zs_stats.get("dynamic_joules", zs_joules))
    cumulative_energy += zs_joules
    cumulative_dynamic_energy += zs_dynamic_joules
    
    logger.record("post_conversion/inference_energy", zs_joules)
    logger.record("post_conversion/inference_energy_dynamic", zs_dynamic_joules)
    logger.record("post_conversion/energy/total", cumulative_energy)
    logger.record("post_conversion/energy/total_dynamic", cumulative_dynamic_energy)
    ...
    if snn_metrics:
        ...
        logger.record("post_conversion/sparsity", snn_metrics.get("sparsity", 0.0) * 100) 
    ...
    logger.record("post_conversion/solved_reward_avg", zero_shot_reward)
    logger.record("post_conversion/solved_success_rate", zs_success_rate)
    logger.record_episode(
        reward=zero_shot_reward, length=zero_shot_len, 
        success=min(100.0, zs_success_rate), source="snn_zero_shot"
    )

    # Side-by-side post-conversion comparison:
    # converted ANN (phase-1), converted SNN (zero-shot), optional external conventional ANN.
    ...
    if conventional_ann_ckpt:
        ...
        try:
            fallback_in_dim = int(config.get("model", {}).get("in_features", 4))
            fallback_hidden_dim = int(config.get("model", {}).get("hidden_dim", 64))
            fallback_act_dim = int(
            ...
            conventional_ann_agent = make_agent(
                actor_type=ActorType.ANN,
                critic_type=CriticType.ANN,
                hidden_dim=hidden_dim_conv,
                dropout=config.get("model", {}).get("dropout", 0.0),
                in_dim=in_dim_conv,
                act_dim=act_dim_conv,
                gamma=ppo_cfg.get("gamma", 0.99),
                critic_informs_actor=bool(config.get("model", {}).get("critic_informs_actor", False)),
                detach_critic_for_actor=bool(config.get("model", {}).get("detach_critic_for_actor", True)),
                normalize_critic_for_actor=bool(config.get("model", {}).get("normalize_critic_for_actor", True)),
            ...
            conventional_ann_wall_ms = float(np.mean(conv_wall_ms)) if conv_wall_ms else 0.0
            ...
            logger.record("comparison/conventional_ann/wall_clock_ms", conventional_ann_wall_ms)
            logger.record("comparison/delta/snn_minus_conventional_ann_reward", float(zero_shot_reward - conventional_ann_reward))
            logger.record("comparison/delta/converted_ann_minus_conventional_ann_reward", float(converted_ann_reward - conventional_ann_reward))
            ...
            conventional_ann_metrics = {
                ...
                "conventional_ann_wall_clock_ms": conventional_ann_wall_ms,
        ...
        configured_ckpt = comparison_cfg.get("conventional_ann_checkpoint")
        if configured_ckpt:
            logger_mod.warning("Configured conventional ANN checkpoint not found: %s", configured_ckpt)
    ...
    if save_ckpt_flag:
        save_checkpoint(
            ...
            num_timesteps=logger.num_timesteps,
            save_dir=ckpt_dir,
            filename="checkpoint_post_conversion_zeroshot.pt",
            mean_reward=zero_shot_reward,
            config=config,
            phase="snn_zero_shot",
            energy_metrics={"total": cumulative_energy, "total_dynamic": cumulative_dynamic_energy},
            extra_data={
                "zero_shot_success_rate": zs_success_rate,
                **comparison_metrics,
            },
        )
        logger_mod.info("Saved zero-shot checkpoint: checkpoint_post_conversion_zeroshot.pt")
    ...
    if record_zero_shot_video:
        try:
            ...
            record_best_agent(
                ...
                seed=seed,
                sticky_action=sticky_cfg.get("eval", True),
                max_steps=500,
                env_kwargs=env_cfg.get("kwargs"),
                partial_obs=env_cfg.get("partial_obs"),
                frame_stack=env_cfg.get("frame_stack"),
    ...
    if snn_finetune_cfg.get("enabled", False) and not zs_is_solved:
        ...
        # --- Resume Logic (Baseline-Aligned for Finetune) ---
        ft_start_update = 1
        ...
        if ft_start_update > ft_updates:
            logger_mod.info(
                "Finetune checkpoint already reached/exceeded configured total_updates; "
                "skipping additional SNN finetuning updates."
            )
        ...
        for update in range(ft_start_update, ft_updates + 1):
            ...
            rollout_joules = float(energy_hook.energy.get("total_joules", 0.0))
            rollout_dynamic_joules = float(energy_hook.energy.get("dynamic_joules", rollout_joules))
            train_full_joules = float(ft_train_stats.get("total_joules", 0.0))
            train_full_dynamic_joules = float(ft_train_stats.get("dynamic_joules", train_full_joules))
            cumulative_energy += train_full_joules
            cumulative_dynamic_energy += train_full_dynamic_joules

            logger.record("energy/train_rollout", rollout_joules)
            logger.record("energy/train_rollout_dynamic", rollout_dynamic_joules)
            logger.record("energy/train_full_update", train_full_joules)
            logger.record("energy/train_full_update_dynamic", train_full_dynamic_joules)
            logger.record("energy/total", cumulative_energy)
            logger.record("energy/total_dynamic", cumulative_dynamic_energy)
            ...
            steps_this_update = train_cfg.get("rollout_length", 2048) * n_envs
            ...
            if update % eval_interval == 0 or update == ft_updates:
                ...
                te_ft, success_rate_ft, avg_len, success_count_ft, n_eval_episodes_ft = ft_trainer.evaluate()
                ft_eval_stats = energy_hook.stop_span(ft_eval_meter, ft_eval_t0)

                eval_joules = float(ft_eval_stats.get("total_joules", 0.0))
                eval_dynamic_joules = float(ft_eval_stats.get("dynamic_joules", eval_joules))
                cumulative_energy += eval_joules
                cumulative_dynamic_energy += eval_dynamic_joules

                logger.record("post_conversion_ft/energy/inference", eval_joules)
                logger.record("post_conversion_ft/energy/inference_dynamic", eval_dynamic_joules)
                logger.record("post_conversion_ft/eval_reward", te_ft)
                logger.record("energy/total", cumulative_energy)
                logger.record("energy/total_dynamic", cumulative_dynamic_energy)
                ...
                if len(test_rewards_ft) >= window_size:
                    rolling_avg_ft = np.mean(test_rewards_ft[-window_size:])
                else:
                    rolling_avg_ft = np.mean(test_rewards_ft)

                logger.record("post_conversion_ft/episode_reward_ma100", float(rolling_avg_ft))
                
                logger.record("post_conversion_ft/solved_reward_avg", float(rolling_avg_ft))
                logger.record("post_conversion_ft/solved_success_rate", float(success_rate_ft))
                ...
                logger.record("post_conversion_ft/n_eval_episodes", float(n_eval_episodes_ft))
                logger.record_episode(
                    reward=te_ft, length=avg_len, 
                ...
                if save_ckpt_flag and (update % ckpt_interval == 0):
                    save_checkpoint(
                        ...
                        num_timesteps=logger.num_timesteps,
                        save_dir=ckpt_dir,
                        filename=f"checkpoint_finetuned_ep{update:05d}.pt",
                        mean_reward=te_ft,
                        config=config,
                        phase="snn_finetune",
                        energy_metrics={'total': cumulative_energy, 'total_dynamic': cumulative_dynamic_energy},
                ...
                if rolling_avg_ft > best_ft_rolling_avg:
                    ...
                    if save_ckpt_flag:
                        save_checkpoint(
                            ...
                            mean_reward=te_ft,
                            config=config,
                            suffix="finetuned_best",
                            energy_metrics={'total': cumulative_energy, 'total_dynamic': cumulative_dynamic_energy},
                            phase="snn_finetune",
                            extra_data={'best_rolling_avg': best_ft_rolling_avg}
                        )
                        logger_mod.info(f"New best SNN model saved! MA100: {best_ft_rolling_avg:.2f}")

                # --- 2. Log Solution (No Break) ---
                # if te_ft >= reward_threshold and not has_logged_ft_solution:
                #     logger_mod.info(f"✅ SNN fine-tuning solved environment at update {update}! Continuing for stability...")
                #     has_logged_ft_solution = True
                
                if (rolling_avg_ft >= reward_threshold) and (success_rate_ft >= success_rate_threshold) and not has_logged_ft_solution:
                    logger_mod.info(f"✅ SNN fine-tuning solved environment at update {update} (MA100: {rolling_avg_ft:.2f})! Continuing for stability...")
                    has_logged_ft_solution = True
                    if record_finetune_solution_video:
                        try:
                            logger_mod.info("Recording Fine-Tuned SNN video (Solution Milestone)...")
                            env_cfg = config.get("env", {})
                            record_best_agent(
                                agent_snn,
                                env_id=env_id,
    ...
    # --- Validation Data Collection (Post-Conversion) ---
    validation_data = {}
    try:
        ...
        while not _budget_reached():
            with torch.no_grad():
                ...
                # Critic value trace for intra-episode value dynamics
        ...
        # Match timing-critic reporting route: deterministic first post-eval episode trace.
        ...
        if collected_actor_logits:
            actor_pot = np.asarray(collected_actor_logits, dtype=float)
            validation_data["actor_output_potentials"] = actor_pot.T
        if collected_actor_spikes:
            actor_spk = np.asarray(collected_actor_spikes, dtype=float)
            validation_data["actor_output_spikes"] = actor_spk.T
