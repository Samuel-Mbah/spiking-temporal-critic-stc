...
import numpy as np
import sys
import glob  # <--- Essential for resume logic
import traceback
import subprocess
...
# from src.training.evaluate import evaluate_snn # (Unused import removed for cleanliness)
...
def run_baseline(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    env_train, env_eval = make_envs(
        ...
        env_kwargs=env_cfg.get("kwargs", {}), # Pass any additional env kwargs if needed
    ...
    # --- 6. RESUME LOGIC ---
    start_update = 1
    # Use the consistent log_dir_root defined earlier
    ...
    if os.path.exists(ckpt_dir):
        ...
        if ckpts:
            ...
            try:
                ...
                # Restore Best Rolling Avg if available (prevents overwriting best model on resume)
                if "best_rolling_avg" in data:
                    best_rolling_avg = data["best_rolling_avg"]

                # Restore VecNormalize stats if present
    ...
    try:
        for update in range(start_update, total_updates + 1):
            ...
            # B. Metrics Logging
            train_rollout_joules = 0.0
            train_rollout_dynamic_joules = 0.0
            latency_ms = 0.0
            
            if hasattr(energy_hook, 'energy') and energy_hook.energy:
                train_rollout_joules = energy_hook.energy.get('total_joules', 0.0)
                train_rollout_dynamic_joules = energy_hook.energy.get('dynamic_joules', train_rollout_joules)
                duration_seconds = energy_hook.energy.get('duration_seconds', 0.0)
                rollout_len = int(train_cfg.get("rollout_length", 2048))
                
                # debug print
                # print(f"DEBUG: rollout_len = {rollout_len}, num_envs = {trainer.env_train.num_envs}")
                
                steps_in_rollout = trainer.env_train.num_envs * rollout_len
                
                if steps_in_rollout > 0:
                    latency_ms = (duration_seconds / steps_in_rollout) * 1000.0
            ...
            logger.record("energy/train_rollout", train_rollout_joules)
            logger.record("energy/train_rollout_dynamic", train_rollout_dynamic_joules)
            ...
            if "latency_mean" in metrics:
                logger.record("latency/mean_ms", metrics["latency_mean"] * 1000.0)
            elif latency_ms > 0.0:
            ...
            if update % eval_interval == 0 or update == total_updates:
                ...
                te, success_rate, avg_len, success_count, n_eval_episodes = trainer.evaluate()
                eval_energy_stats = energy_hook.stop_span(eval_meter, eval_t0)
                
                eval_joules = eval_energy_stats.get('total_joules', 0.0)
                eval_dynamic_joules = eval_energy_stats.get('dynamic_joules', eval_joules)
                ...
                # Debugging info
                # print(f"DEBUG: History Length: {len(test_rewards)} | Window: {window_size}")
                ...
                if save_ckpt_flag and (update % ckpt_interval == 0):
                    last_checkpoint_path = save_checkpoint(
                        ...
                        save_dir=ckpt_dir, # Use consistent path
                        mean_reward=te,
                        config=config,
                        # Pass best_rolling_avg so we can recover it on resume
                ...
                # We save if it's the best we've seen, but we DO NOT stop.
                ...
                # 3. Log "Solved" Milestone (Informational only)
                if is_solved and not has_logged_solution:
                    logger_mod.info(f"✅ Environment solved at update {update} (Avg: {rolling_avg:.2f})! Continuing training for stability...")
                    has_logged_solution = True 
                    
                    # Record a "solution video" once, but keep training
                    try:
