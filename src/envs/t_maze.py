...
# Only enable LaTeX if a TeX distribution is available; otherwise keep Matplotlib's default.
...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, MAP=MAP, length=3, random_length=False, active=False, fix_start=True, seed=None, max_episode_step=1000000, fully_obs=False, goal_obs=False, continual=False, render_mode=None):   
    ...
    def render(self):    
        ...
        if self.render_mode == 'rgb_array':
            ...
            if hasattr(canvas, "buffer_rgba"):
                # Preferred path for modern Matplotlib backends.
                ...
                raise RuntimeError("Matplotlib canvas does not provide RGB buffer extraction methods.")

            # RecordVideo/MoviePy requires all frames in a clip to have identical size.
            if self._render_frame_shape is None:
                self._render_frame_shape = img.shape
            elif img.shape != self._render_frame_shape:
                target_h, target_w, target_c = self._render_frame_shape
                fixed = np.zeros((target_h, target_w, target_c), dtype=np.uint8)
                src_h, src_w, src_c = img.shape
                copy_h = min(target_h, src_h)
                copy_w = min(target_w, src_w)
                copy_c = min(target_c, src_c)
                fixed[:copy_h, :copy_w, :copy_c] = img[:copy_h, :copy_w, :copy_c]
                img = fixed
    ...
    def gen_fig(self):
        fig, ax = plt.subplots(figsize=(self.n, self.m), dpi=100, facecolor='w', edgecolor='k')
        params = {'font.size': 40}
        ...
        s = 0.98
        ax.imshow(self._gridmap_to_img(), origin="upper", extent=[s, self.n-s, self.m-s, s])
